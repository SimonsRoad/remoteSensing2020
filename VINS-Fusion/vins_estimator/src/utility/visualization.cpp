...
std::string groundTruthFolder = "../remoteSensing2020/src/result/groundTruth.csv";
std::string pdopFolder = "../remoteSensing2020/src/result/pdop.csv";
FILE* cova  = fopen( "../remoteSensing2020/src/result/cova.csv", "w+");
...
void span_bp_callback(const novatel_msgs::INSPVAXConstPtr& fix_msg)
{
   ...
   //  fflush(error_xyfile);
    ...
    // write to span file------begain

    time_now = time_now+1;

  //  std::ofstream foutGt(groundTruthFolder, std::ios::out);
  //  fout1.close();
  ...
  // write to span file------end

  //write to vio file------begain
    ...
    //write to vio file------end

    /* write pdop to csv */
    ofstream foutpdop(pdopFolder, ios::app);
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_pdop = n.advertise<nav_msgs::Odometry>("Pdop",1000); //publish pdop 
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        m_cov = estimator.m_sqrt.inverse();
        cov_value = m_cov(0,0);
        std::cout<< ".........m_sqrt....... "<< estimator.m_sqrt<<std::endl;
        std::cout<< ".........cov_value....... "<< cov_value <<std::endl;
        ...
        //******************* write result to vio file, 20200110 by bxw, remove this part to span_bp_callback**********************
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // Eigen::MatrixXd featureCameraPose;
    std::vector<Eigen::Matrix<double, 1,6>> featureImuPose;
  //  std::cout<< "------estimator.f_manager.feature-------" << estimator.f_manager.feature.size()<< std::endl;
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric[0] * pts_i + estimator.tic[0]) + estimator.Ps[imu_i]; // camera to IMU frame, then IMU to the world frame
        ...
        Eigen:Vector3d imuPose = estimator.Ps[imu_i];
        ...
        // std::cout<< "------used_num-------" << used_num<< std::endl;
    }
   ...
   // calculate the PDOP
    ...
    std::cout<< "*****g_pdop********" << g_pdop<< std::endl;


    //  std::cout<< "Pub point cloud" << count<< std::endl;
